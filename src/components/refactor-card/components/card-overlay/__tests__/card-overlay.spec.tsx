...
describe("Card Overlay Component", () => {
  test("Card Overlay should render with the right properties", () => {
    const { getByTestId, getByText } = render(<CardOverlay {...cardOverlayMock}/>);

    const cardOverlay = getByTestId("card-overlay");
    expect(cardOverlay).toBeInTheDocument();

    const plantTitle = getByText(cardOverlayMock.data.name.toUpperCase());
    expect(plantTitle).toBeInTheDocument();

    const waterIcons = cardOverlay.querySelectorAll(".category-level")[0].querySelectorAll("svg");
    expect(waterIcons.length).toBe(5);

    const careIcons = cardOverlay.querySelectorAll(".category-level")[1].querySelectorAll("svg");
    expect(careIcons.length).toBe(5);

    const sizeIcons = cardOverlay.querySelectorAll(".category-level")[2].querySelectorAll("svg");
    expect(sizeIcons.length).toBe(3);

    const seasonIcons = cardOverlay.querySelectorAll(".category-level")[3].querySelectorAll("svg");
    expect(seasonIcons.length).toBe(4);
  });

  test("Card Overlay should not render if data is null or undefined", () => {
    const { container } = render(<CardOverlay data={null as unknown as PlantDataType}/>);
  ...
  test("Card Overlay should display the correct number of active icons", () => {
    ...
    const waterActiveIcons = cardOverlay.querySelectorAll(".category-level")[0].querySelectorAll(".active");
    expect(waterActiveIcons.length).toBe(cardOverlayMock.data.water.level);

    const careActiveIcons = cardOverlay.querySelectorAll(".category-level")[1].querySelectorAll(".active");
    expect(careActiveIcons.length).toBe(cardOverlayMock.data.care.level);

    const sizeActiveIcons = cardOverlay.querySelectorAll(".category-level")[2].querySelectorAll(".active");
    expect(sizeActiveIcons.length).toBe(1);

    const seasonActiveIcons = cardOverlay.querySelectorAll(".category-level")[3].querySelectorAll(".active");
