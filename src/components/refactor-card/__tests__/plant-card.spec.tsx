...
describe("Plant Card Component", () => {
  test("Plant Card should render with the right properties", () => {
    const { getByText,getByAltText, getByTestId } = render(<PlantCard {...plantCardMock}/>);
  ...
  test("Plant Card should not be rendered if data is undefined or null", () => {
    const { container } = render(<PlantCard {...plantCardMock} plant={null as unknown as PlantDataType}/>);
  ...
  test("Plant Card should call onLikeClick when FavoriteButton is clicked", () => {
    ...
    const { getByTestId } = render(<PlantCard {...plantCardMock} onLikeClick={onLikeMock}/>);
